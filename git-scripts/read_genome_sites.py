...
file_dir = Path(
    # Allow overriding when reproducing the author's original folder layout.
    # Example: export GENOME_BED_DIR=".../data/genome"
    # (This script reads BEDs but doesn't create them.)
    #
    # NOTE: keep trailing slash behavior compatible with the old `file_dir + name` pattern
    # by treating it as a directory Path.
    os.environ.get("GENOME_BED_DIR", str(_DEFAULT_BED_DIR))
)
